...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer, ctc_criterion):
        epoch_loss = 0
        model.train()
        # inputs = (torch.randn((1,50,300)).cuda(),torch.randn(1,50,74).cuda())
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if batch_chunk > 1:
                ...
                for i in range(batch_chunk):
                    ...
                    # flops = FlopCountAnalysis(net, (text_i, audio_i))
                    # print(flops.total())
    ...
    def evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                #         attn = attn_weights
                #     else:
                #         attn = torch.cat((attn_weights,attn))
                #     print(attn.shape)
                if hyp_params.dataset == 'iemocap':
                    preds = preds.view(-1, 2)
                    eval_attr = eval_attr.view(-1)
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        writer.add_scalars('run_14h', {'val':val_loss,
